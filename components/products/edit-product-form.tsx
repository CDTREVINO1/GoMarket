...
export default function EditProductForm({
  ...
  const [productImages, setProductImages] = useState<Image[]>(product.images)
  ...
  async function deleteImages() {
    ...
    for (let image of deletedImages) {
      ...
      const endpoint = clientEnv.NEXT_PUBLIC_CLOUDINARY_DESTROY_URL
      const data = await fetch(endpoint, {
        method: "POST",
        body: formData,
      }).then((res) => res.json())
      ...
      await deleteImageFromDatabase(product.id, image.public_id)
    }
  }

  const onSubmit = async (data: ProductFormInputs) => {
    const updatedProduct: UpdateProductInput = { ...data, id: product.id }
    if (files.length > 0) {
      const images = await uploadImages()
      updatedProduct.images = images.map((img) => img.url)
    }
